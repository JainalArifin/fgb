...
class AddBlog extends Component {
    ...
    handleSubmit = (e) => {
        e.preventDefault()
        const { title, imgUrl, editorState, author, date } = this.state
        // console.log(title, ' <----- title')
        // console.log(imgUrl, ' <----- imgUrl')
        // console.log(editorState, ' <----- editorState')
        // console.log(author, ' <----- author')
        // console.log(date, ' <----- date')
        if(title === '' || imgUrl === '' || editorState === '' || author === '' || date === ''){
            // console.log(' <-------- 1')
            this.setState({
                handleEmpty: true
            })
    ...
    render() {
        const { editorState, title, imgUrl, date, author, handleEmpty, startDate } = this.state;
        return (
            <div>
                ...
                {handleEmpty === true && (
                    <Alert
                        message="Maaf"
                        description="input harus di isi semua"
                        type="error"
                        showIcon
                    />
                )}
                ...
                >
                    ...
                    <FormGroup>
                        <Label>Title</Label>
                        <Input type="text" name="title" className="input-style-addBlog title" placeholder=" ..write title in here"
                            value={title}
                            onChange={this.handleChange}
                        />
                    </FormGroup>
                    ...
                    <FormGroup>
                        ...
                        <Editor
                            ...
                            value={editorState}
                            toolbar={{
                                image: { uploadCallback: this.uploadImageCallBack, alt: "image" },
                            }}
